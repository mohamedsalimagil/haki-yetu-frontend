            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step.id <= currentStep
                ...
                {index < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-4 mt-[-20px] ${step.id < currentStep ? 'bg-[#1E40AF]' : 'bg-gray-200'
                    }`} />
                )}
              </React.Fragment>
            ))}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {['Today 2:00 PM', 'Today 4:00 PM', 'Tomorrow 10:00 AM', 'Tomorrow 2:00 PM'].map((slot, index) => (
                    <button
                      ...
                      className={`p-3 border rounded-lg text-sm font-medium transition ${selectedTimeSlot === slot
          ...
          <div className="flex justify-between items-center mt-8">
            ...
            <div className="flex items-center gap-2">
              {steps.map((step) => (
                <div
                  key={step.id}
                  className={`w-3 h-3 rounded-full ${step.id <= currentStep ? 'bg-[#1E40AF]' : 'bg-gray-300'
                    }`}
                />
              ))}
            </div>
