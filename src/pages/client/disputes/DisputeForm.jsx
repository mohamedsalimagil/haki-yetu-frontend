...
const DisputeForm = () => {
  ...
  // TODO: Replace with API call to fetch dispute categories
  ...
  const handleFileUpload = (files) => {
    // Only keep the File objects, we don't need formatted size for upload
  ...
  const handleSubmit = async (e) => {
    ...
    if (!order) {
      alert("No order selected for dispute.");
      return;
    }
    ...
    try {
      ...
      data.append('resolution_preference', formData.resolutionMethod);

      // Append first file as evidence (backend supports one file currently, can be enhanced)
      if (uploadedFiles.length > 0) {
        data.append('evidence', uploadedFiles[0]);
      }

      const response = await clientService.createDispute(data);

      navigate('/client/disputes/success', {
        state: {
          disputeId: response.reference_id,
      ...
      alert(error.response?.data?.error || "Failed to submit dispute. Only clients can submit disputes for their own orders.");
    } finally {
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-8">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <label className={`relative flex flex-col items-center p-6 border-2 rounded-lg cursor-pointer transition ${formData.resolutionMethod === 'email'
                    ...
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`rounded-lg p-3 mb-3 ${formData.resolutionMethod === 'email' ? 'bg-blue-100' : 'bg-gray-100'
                      }`}>
                      <Mail className={`w-6 h-6 ${formData.resolutionMethod === 'email' ? 'text-blue-600' : 'text-gray-600'
                        }`} />
                    </div>
                  ...
                  <label className={`relative flex flex-col items-center p-6 border-2 rounded-lg cursor-pointer transition ${formData.resolutionMethod === 'phone'
                      ? 'border-blue-600 bg-blue-50'
                      : 'border-gray-200 hover:border-gray-300'
                    }`}>
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`rounded-lg p-3 mb-3 ${formData.resolutionMethod === 'phone' ? 'bg-blue-100' : 'bg-gray-100'
                      }`}>
                      <Phone className={`w-6 h-6 ${formData.resolutionMethod === 'phone' ? 'text-blue-600' : 'text-gray-600'
                        }`} />
                    </div>
                  ...
                  <label className={`relative flex flex-col items-center p-6 border-2 rounded-lg cursor-pointer transition ${formData.resolutionMethod === 'chat'
                      ? 'border-blue-600 bg-blue-50'
                      : 'border-gray-200 hover:border-gray-300'
                    }`}>
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`rounded-lg p-3 mb-3 ${formData.resolutionMethod === 'chat' ? 'bg-blue-100' : 'bg-gray-100'
                      }`}>
                      <MessageSquare className={`w-6 h-6 ${formData.resolutionMethod === 'chat' ? 'text-blue-600' : 'text-gray-600'
                        }`} />
                    </div>
