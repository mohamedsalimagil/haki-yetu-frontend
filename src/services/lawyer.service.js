...
const lawyerService = {
  ...
  getAllLawyers: async (filters = {}) => {
    ...
    const response = await api.get(`/api/marketplace/lawyers?${queryParams.toString()}`);
    return response.data.lawyers || [];
  },

  getLawyerById: async (lawyerId) => {
    const response = await api.get(`/api/marketplace/lawyers/${lawyerId}`);
    return response.data;
  },
  ...
  bookConsultation: async (lawyerId, bookingData) => {
    ...
    const response = await api.post('/api/documents/consultations/book', payload);
    return response.data;
  },
  ...
  rescheduleConsultation: async (consultationId, newDate, newTimeSlot, reason) => {
    ...
    const response = await api.put(`/api/documents/consultations/${consultationId}/reschedule`, payload);
    return response.data;
  },
  ...
  cancelConsultation: async (consultationId, reason) => {
    ...
    const response = await api.put(`/api/documents/consultations/${consultationId}/cancel`, payload);
    return response.data;
  }
};
